...
def parse_just_copy():
    parser = Parser(description = 'description: this script copies .mp3 file(s) from a folder to a Shokz device',
                    epilog      = 'example usage: python just_copy.py -f ~/Desktop/Songs -s /Volumes/OpenSwim')
    parser.add_argument('-f', '--folder', required = True, help = 'the path to a folder with .mp3 files to copy, i.e. ~/Desktop/Songs')
    parser.add_argument('-s', '--shokz',  required = True, help = 'the path to the Shokz folder mounted on the machine, i.e. /Volumes/OpenSwim')
